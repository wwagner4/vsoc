  ...
  def apply(sens: Sensors): Option[Array[Double]] = {
    ...
    sens.getFlagsLeft.asScala.foreach { flag =>
      flag match {
        ...
        case (Flag.FLAG_OWN_30, dirdist) => re(23) = handle(dirdist)
        case (Flag.FLAG_OWN_20, dirdist) => re(22) = handle(dirdist)
        case (Flag.FLAG_OWN_10, dirdist) => re(21) = handle(dirdist)
        case (Flag.FLAG_OTHER_10, dirdist) => re(20) = handle(dirdist)
        case (Flag.FLAG_OTHER_20, dirdist) => re(19) = handle(dirdist)
        case (Flag.FLAG_OTHER_30, dirdist) => re(18) = handle(dirdist)
        case (Flag.FLAG_OTHER_40, dirdist) => re(17) = handle(dirdist)
