...
object Data01Dia {
  ...
  def createDiaConfig(
    ...
    implicit val creator: VizCreator[Viz.XY] = createCreator(workDir, diaDir)
    val dia: Viz.Dia[Viz.XY] = createDia(cfg.trainings, cfg.id, workDir, dataPoints, diaConfs, xRange, yRange, None)
  ...
  def createDiaWorkDir(
    ...
    def extractTrainings: Seq[Seq[ConfigTrainGa]] = {
      require(Files.exists(workDir))
      require(Files.isDirectory(workDir))
      ...
      Files.list(workDir).iterator().asScala.toSeq.map { file =>
    ...
    val dias: Seq[Viz.Diagram[Viz.XY]] = trainings.zipWithIndex.map {
      case (tr, i) =>
        ...
        createDia(tr, s"dia$i", workDir, dataPoints, diaConfs, xRange, yRange, tit)
    }
